...
inline void select_iterator(InputParams& IP, Grid& grid, Source& src, IO_utils& io, 
    ...
    if (!is_teleseismic){
        if (IP.get_sweep_type() == SWEEP_TYPE_LEGACY) {
            ...
            if (IP.get_stencil_order() == 1){
                if (IP.get_stencil_type() == UPWIND){
                    ...
                    It = std::make_unique<Iterator_level_1st_order_upwind>(IP, grid, src, io, first_init, is_teleseismic, is_second_run);
                } else {
        ...
        if (IP.get_sweep_type() == SWEEP_TYPE_LEGACY) {
            ...
            if (IP.get_stencil_order() == 1){
                if (IP.get_stencil_type() == UPWIND){
                    It = std::make_unique<Iterator_level_1st_order_upwind_tele>(IP, grid, src, io, first_init, is_teleseismic, is_second_run);
                } else {
