...
Optimizer_bfgs::Optimizer_bfgs() {
    ...
    array_3d_forward   = allocateMemory<CUSTOMREAL>(n_total_loc_grid_points, 1);
    array_3d_backward  = allocateMemory<CUSTOMREAL>(n_total_loc_grid_points, 2);

    sk_s   = allocateMemory<CUSTOMREAL>(n_total_loc_grid_points, 3);
    sk_xi  = allocateMemory<CUSTOMREAL>(n_total_loc_grid_points, 4);
    sk_eta = allocateMemory<CUSTOMREAL>(n_total_loc_grid_points, 5);
    yk_s   = allocateMemory<CUSTOMREAL>(n_total_loc_grid_points, 6);
    yk_xi  = allocateMemory<CUSTOMREAL>(n_total_loc_grid_points, 7);
    yk_eta = allocateMemory<CUSTOMREAL>(n_total_loc_grid_points, 8);
}

Optimizer_bfgs::~Optimizer_bfgs() {
    delete[] array_3d_forward;
    delete[] array_3d_backward;
...
void Optimizer_bfgs::get_model_dif(Grid& grid, IO_utils& io, int& i_inv){
    ...
    grid.set_array_from_vis(array_3d_forward);
    io.read_vel(grid, i_inv);
    grid.set_array_from_vis(array_3d_backward);
